...
function formSubmit(){
  ...
  if (this.action.match(/login$/)) {
    ...
    $(this).ajaxSubmit({"success":getDisplayFunc($(this).closest(".instanceContainer"))});
  }
  return false;
}
...
function colEditKeypress(e){
  if (e.keyCode==27){
    toggleColStat.apply($(this).parents(".editing"));
  }
}
function instanceActionClick(){
  $.get($(this).closest(".instance").attr("action")+'/'+$(this).attr('class').replace(/.* /,""),"",getDisplayFunc($(this).closest('.content')));
  return false;
}
function deleteResource(){
}
function getDisplayFunc(target){
  if ($(target).hasClass("search")) {
    target=$(target).closest(".section").children(".content");
  }
  if ($(target).hasClass("instanceContainer")) {
    return function(json) { $(target).replaceWith(json.toDom()); }
  } else {
    return function(json) { $(target).html(json.toDom()); }
  }
}
...
nodeTypes={
  "resource":function(r){
    var res=["a",{"href":r.attrs.url},r.attrs.title];
    if (r.attrs.deletable){
      return ["div",{"class":"resourceContainer"},[res,["input",{"class":"delete "+r.attrs.title,"type":"button","value":"x"}]]]
    } else {
      return res
    }
  },
  ...
  "instance":function(i) {
    ...
    if (attrs.search){
      attrs.new_rec=true;
      var frm=["form",{"action":"/"+attrs.resource,"method":"get","class":"search"}];
    } else {
      var frm=["form",{"action":"/"+attrs.resource,"method":"post","class":attrs.resource+"Instance instance"}];
    }
        ...
        if (col[RESTRICTIONS_INDEX] & RESTRICT_RO) {
          ...
          if (! attrs.new_rec) {
            ...
            if (ctl[1].constructor==Array){ //many to one
              ...
              show_val=ctl[1].value || ctl[2] //textare value
            }
    ...
    if (attrs.search){
      ...
      contents.push(["input",{"type":"submit","value":(attrs.resource=="login" ? "login" : "save"),"class":"submit " + (attrs.new_rec ? "" : "hidden")}]);
    }

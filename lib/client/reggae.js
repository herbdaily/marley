  ...
  var nodeTypes={
    ...
    "instance":function(i) {
      ...
      if ((! attrs.new_rec) && (actions=attrs.actions)){
        contents.push(["div",{"class":"instanceActions"},
            pl.type(actions.get,'str') ? ["a",{"class":"instanceAction "+actions.get},actions.get.humanize()] :
            pl.map(actions.get,function(action){
    ...
    'arr' : function(){
      if(this.isNodeSpec()) {
        if (node_type=nodeTypes[this.nodeType]) {
          ...
          if(this.content){
            var content=Reggae.call(this.content).toDom();
        ...
        return pl.map(this, function(e){return Reggae.call(e).toDom()})
      }
    }
  }
  this.toDom=domFuncs[pl.type(this)];
  if (target) {
    var t=pl(target);
    var d=this.toDom();
    if (pl.type(d,'arr')){

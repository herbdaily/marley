...
j={
  getDisplayFunc:function(target,func){
    return function(json) { pl(target)[func || "replace"](j.toDom(json));j.bindEvents() }
  },
  bindEvents:function() {
    ...
    pl('ul.menu li a').bind('click',j.menuClick);
    pl('form.instance, form.search').bind('submit',j.formSubmit);
    pl('.editable').bind('click',j.toggleColStat);
    pl('.editing input').bind('keypress',j.colEditKeypress);
    pl('a.instanceAction').bind('click',j.instanceActionClick);
    pl('.instanceContainer .delete').bind('click',j.deleteResource);
  },

...
{
    ///////////////////////////////////////////////////////////////////////////// SETUP
    var RESTRICT_HIDE = 1, NAME_INDEX = 1;
    var RESTRICT_RO = 2, TYPE_INDEX=PROPS_INDEX = 2, RESTRICTIONS_INDEX = 2;
    var VALUE_INDEX = 3;
    var RESTRICT_REQ = 4;
    var nodeTypes =
    {
        "uri": function(r)
        {
            var uri = ["a", {"href": r.attrs.url}, r.attrs.title];
            if (r.attrs.deletable)
            {
                return ["div", {"class":"uriContainer"}, [uri, ["input", {"class": "delete " + r.attrs.title, "type": "button", "value": "x"}]]];
            }
            else
            {
                return uri;
        ...
        "instance": function(i)
        {
            ...
            {
                attrs.new_rec = true;
                frm = ["form", {"action": "/" + attrs.uri, "method": "get", "class": "search"}];
            }
            else
            {
                frm = ["form", {"action": "/" + attrs.uri, "method": "post", "class": attrs.uri + "Instance instance"}];
            }
            ...
            {
                var props = {"id": attrs.uri + "__" + col[1]};
            ...
            {
                contents.push(["input", {"type": "submit", "value": (attrs.uri == "login" ? "login" : "save"), "class": "submit " + (attrs.new_rec ? "" : "hidden")}]);
            }
            if ((!attrs.new_rec) && (attrs.instance_get_actions))
            {
                contents.push(
                    [
                        "div",
                        {"class": "instanceActions"},
                        $.map(attrs.instance_get_actions, function(action)
                        {
    ...
    function formSubmit()
    {
        ...
        {
            $.ajax({"url": "/main_menu", "username": $('#login__name').val(), "password": $('#login__password').val(), "success": getDisplayFunc('body')});
        }
        else
        {
    ...
    function showError(e, r, s)
    {
        ...
        try
        {
            ...
            if(errArr[0] == 'validation')
            {
                $.each(errArr[1], function(col, msg)
                {
    ...
    function initialize()
    {
        ...
        $('.uriContainer .delete').live('click', deleteResource);
    }

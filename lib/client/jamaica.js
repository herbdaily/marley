...
pl(function(){
  j.ajax_defaults.success.call('body',_jamaica_json);
...
j={
  ajax_defaults:{
    ...
    success:function(json){ pl(this).html(j.toDom(json)); },
    error:function(stat,json){
      var err=JSON.parse(json);
      var attrs=err[1];
      var frm_id=this.id;
      if (attrs.error_type=='validation') {
        pl('#'+frm_id+' input, #'+frm_id+' textarea').each(function(){
          if (detail=attrs.error_details[this.id.replace(/.*__/,'')]){
            pl(this).addClass('errorCol');
          }
        })
      }
    },

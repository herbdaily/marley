...
pl(function(){
  j.getDisplayFunc('body')(_jamaica_json);
});

j={
  default_ajax:function(){
    return new pl.Ajax({
      dataType:'json',
      callbacks:{
        load:function(){pl('body').css('cursor','wait')},
        error:[function(){pl('body').css('cursor','default');},j.showError],
        success:function(){pl('body').css('cursor','default');}
      }
    });
  },
  getDisplayFunc:function(target,func){
    return function(json) { 
      pl(target)[func || "html"](j.toDom(json));j.bindEvents(target);
    }
  },
  bindEvents:function(target) {
    pl('ul.menu li a',target).bind('click',j.menuClick);
    pl('form.instance, form.search',target).bind('submit',j.formSubmit);
    pl('.editable',target).bind('click',j.toggleColStat);
    pl('.editing input',target).bind('keypress',j.colEditKeypress);
    pl('a.instanceAction',target).bind('click',j.instanceActionClick);
    pl('.instanceContainer .delete',target).bind('click',j.deleteResource);
  },
  showError:function(stat,json) {
    var err=JSON.parse(json);
    var err_type=err.shift();
    var attrs=err.shift();
    var frm_id=this.id;
    if (attrs.error_type=='validation') {
      console.log(attrs.error_details)
      pl.each (attrs.error_details, function(i,detail){
        pl('.'+frm_id+' id$=['+detail[0]+']').addClass('errorCol');
      })
    }
  },
  menuClick:function(json){
    j.default_ajax().get({url:this.href,success:j.getDisplayFunc('#'+pl(this).parent().attr('class').replace(/__nav$/,'__content'))});
    return false;
  },
  formSubmit:function(json){
    j.default_ajax().post({url:this.action,data:pl.serialize(this.id),success:j.getDisplayFunc(this.id)});
    return false;
  },
  toggleColStat:function(json){
  },
  colEditKeypress:function(json){
  },
  instanceActionClick:function(json){
  },
  ...
  nodeTypes:{
    ...
    "instance":function(i) {
      ...
      if (attrs.search){
        ...
        contents.push(["input",{"type":"submit","value":(attrs.name=="login" ? "login" : "save"),"class":"submit " + (attrs.new_rec ? "" : "hidden")}]);
      }

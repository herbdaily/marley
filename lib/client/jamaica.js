...
pl(function(){
  ...
  pl('body').bind('ajaxStart',function(){pl('body').css('cursor','wait')});
...
j={
  ...
  menuClick:function(json){
    pl.get({url:this.href,dataType:'json',success:j.getDisplayFunc('#'+pl(this).parent().attr('class').replace(/__nav$/,'__content'))})
    return false;
  },
  formSubmit:function(json){
    pl.post({url:this.action,dataType:'json',data:pl.serialize(this.id),success:j.getDisplayFunc(this.id)});
    return false;
  },

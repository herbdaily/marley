...
$(function(){
  ...
  $('.instanceContainer .delete').live('click',deleteResource);
});
...
function formSubmit(){
  //TEMPORARY
  if ($(this).attr('class').match(/login/)) {
    $.ajax({"url":this.action,"username":$('#login__name').val(),"password":$('#login__password').val(),"success":getDisplayFunc('body')});
  } else {
